...
class vector {
 ...
 public:
  ...
  ~vector() {
    for (size_type i = 0; i < len; i++) allocator.destroy(&head[i]);
  ...
  iterator end() { return iterator(head + len); }

  const_iterator end() const { return const_iterator(head + len); }
  ...
  reference operator[](size_type n) { return head[n]; }

  const_reference operator[](size_type n) const { return head[n]; }

  reference at(size_type n) {
    if (n >= len) throw std::out_of_range("vector");
    return head[n];
  }
  ...
  reference front() { return head[0]; }

  const_reference front() const { return head[0]; }

  reference back() { return head[len - 1]; }

  const_reference back() const { return head[len - 1]; }
  ...
  iterator erase(iterator first, iterator last) {
    ...
    for (size_type i = 0; i < n; i++) allocator.destroy(&head[first_id + i]);
...
class vector_iterator {
 ...
 public:
  ...
  reference operator[](difference_type n) const { return *(elem + n); }
  ...
  friend ptrdiff_t operator-(const vector_iterator<T> &lhs,
...
class const_vector_iterator {
 ...
 public:
  ...
  friend void swap(const const_vector_iterator &a,
                   const const_vector_iterator &b) {
