...
namespace ft
{
	...
	class map
	{
		public:
			...
			typedef map_iterator<key_type, value_type, value_compare, key_compare> iterator;
			typedef const_map_iterator<key_type, value_type, value_compare, key_compare> const_iterator;
		...
		public:
			explicit map (const key_compare& comp = key_compare(),
            	const allocator_type& alloc = allocator_type()):allocator(alloc),comp(comp),vcomp(value_compare(comp))
			{
				root = NULL;
			}
			...
			map (const map& x):allocator(x.allocator),comp(x.comp),vcomp(value_compare(x.comp))
			{
				root = NULL;
	...
	struct tree_node
	{
		...
		tree_node(tree_node *left = NULL, tree_node *right = NULL, T *value = NULL):left(left),right(right),value(value)
		{
			set_height();
		}
	...
	class map_iterator
	{
		...
		public:
			map_iterator():node(NULL),vcomp(value_compare(key_compare())),comp(key_compare())
			{

			}

			map_iterator(tree_node* node, tree_node* root):node(node),vcomp(value_compare(key_compare())),comp(key_compare())
			{
				if (node == NULL)
					q.push(root);
				else
			...
			map_iterator(const key_type &k, tree_node* root):node(root),vcomp(value_compare(key_compare())),comp(key_compare())
			{
				while (node != NULL)
				{
			...
			map_iterator(const map_iterator &other):node(other.node),q(other.q),vcomp(value_compare(key_compare())),comp(key_compare())
			{
	...
	class const_map_iterator
	{
		...
		public:
			const_map_iterator():node(NULL),vcomp(value_compare(key_compare())),comp(key_compare())
			{

			}

			const_map_iterator(tree_node* node, tree_node* root):node(node),vcomp(value_compare(key_compare())),comp(key_compare())
			{
				if (node == NULL)
					q.push(root);
				else
			...
			const_map_iterator(const key_type &k, tree_node* root):node(root),comp(key_compare()),vcomp(value_compare(key_compare()))
			{
				while (node != NULL)
				{
					if (comp(k, node->value->first))
			...
			const_map_iterator(const const_map_iterator &other):node(other.node),q(other.q),vcomp(value_compare(key_compare())),comp(key_compare())
			{
